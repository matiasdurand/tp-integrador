...
public class GrafoPlantas extends Grafo<Planta> {
	...
	private Vertice<Planta> getAcopioInicial() {
		return vertices.stream().filter(v->v.getValor().getId().equals(1)).collect(Collectors.toList()).get(0);
	}
	
	private Vertice<Planta> getAcopioFinal() {
		return vertices.stream().filter(v->v.getValor().getId().equals(2)).collect(Collectors.toList()).get(0);
	}
    	...
    	for(Vertice<Planta> ady: adyacentes) {
    		...
    		if(ady.equals(v2)) {
    			...
    			if(!copiaMarcados.contains(ady)) {
    				...
    				buscarCaminosAux(ady, v2, copiaMarcados, caminos, d+a.getDistancia(), t+a.getTiempo(), Math.min(p, a.getPesoMax()));
    		    }
    		}
	...
	private List<Vertice<Planta>> dirigidos(Vertice<Planta> v) {
		...
		for(Arista<Planta> a: aristas) if(a.getFin().equals(v)) dirigidos.add(a.getInicio());
